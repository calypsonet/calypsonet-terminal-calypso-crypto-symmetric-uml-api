...
| https://docs.terminal-api.calypsonet.org/calypsonet-terminal-calypso-card-uml-api/[_Terminal Calypso Card API Specification_], version {ver-cna-tcc}, {organization}.
|===
...
[[sec_pkg_calypso_crypto_symmetric]]

[[sec_pkg_calypso_crypto_symmetric_spi]]
...
| `VERSION` | `string` | 0.1  | String representation of the version of the API implemented by the conforming binding (e.g. `"0.1"`). The value MUST be a dotted decimal of the form `MAJOR.MINOR`.
|===

[NOTE]
====
The `SymmetricCryptoApiProperties` type MUST NOT be instantiated.
====
...
| `cardKeyDiversifier`   -- the card key diversifier to use for the coming cryptographic computations. +
  `useExtendedMode`      -- requests the use of the extended mode if supported by the crypto service. +
