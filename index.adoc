...
The Calypso Card API (<<ref_cna_tcc,CNA-TCC-API>>) declares marker interfaces through which the objects of a
symmetric crypto module are handed over to the card API. Those markers declare no operation: their content
is the one defined by this specification. Every object implementing a type of the right column MUST
therefore also implement the marker of the left column.

[cols="^.^1,^.^1", options="header"]
|===
| CNA-TCC-API marker                             | CNA-TCCS-API type
| `SymmetricCryptoCardTransactionManagerFactory` | <<type_SymmetricCryptoCardTransactionManagerFactorySpi,`SymmetricCryptoCardTransactionManagerFactorySpi`>>
|===
...
| Properties and errors consumed by both sides of the SPI.

| `calypso.crypto.symmetric.spi`                  | SPI
| Interfaces implemented by the symmetric crypto module, and the data they return.
|===
...
| `VERSION` | `String` | 0.1 | String representation of the version of the API implemented by the conforming binding.
  The value is a dotted decimal of the form `MAJOR.MINOR`; it is `"{api-version}"` for this revision of the specification.
|===

=== Data

[[type_SvCommandSecurityData]]
==== SV Command Security Data
...
| Since      | 0.1
| Purpose    | Carries the security data computed by the crypto module for an SV command (Load / Debit / Undebit), returned by <<op_SymmetricCryptoCardTransactionManagerSpi_computeSvCommandSecurityData,`SymmetricCryptoCardTransactionManagerSpi.computeSvCommandSecurityData`>>.
|===
...
| --
| 0.1
| Serial number to be placed in the *SV Load/Debit/Undebit* command request.

| `transactionNumber`
| `ByteArray`
| --
| 0.1
| Transaction number to be placed in the *SV Load/Debit/Undebit* command request.

| `terminalChallenge`
| `ByteArray`
| --
| 0.1
| Terminal challenge to be placed in the *SV Load/Debit/Undebit* command request.

| `terminalSvMac`
| `ByteArray`
| --
| 0.1
| Terminal SV MAC to be placed in the *SV Load/Debit/Undebit* command request.
|===
...
`{nbsp}{nbsp}{nbsp}{nbsp}transactionAuditData: MutableList<ByteArray>` +
...
| `cardKeyDiversifier`   -- the card key diversifier to use for the coming cryptographic computations. +
  ...
  `transactionAuditData` -- the list to which the crypto module appends the transaction audit data it records.
...
`{nbsp}{nbsp}{nbsp}{nbsp}svGetRequest: ByteArray,` +
`{nbsp}{nbsp}{nbsp}{nbsp}svGetResponse: ByteArray,` +
`{nbsp}{nbsp}{nbsp}{nbsp}svCommandPartialRequest: ByteArray` +
`) -> SvCommandSecurityData`
| Since          | 0.1
| Description
| Computes the security data needed to complete an *SV Load/Debit/Undebit* command from the supplied *SV Get*
  exchange and partial command request.
| Parameters
| `svGetRequest`            -- the *SV Get* ingoing command data. +
  `svGetResponse`           -- the *SV Get* outgoing command data. +
  `svCommandPartialRequest` -- the partial *SV Load/Debit/Undebit* ingoing command data, to which the computed security data is later appended.
| Returns        | A <<type_SvCommandSecurityData,`SvCommandSecurityData`>> carrying the security data to place in the *SV Load/Debit/Undebit* command request.
| Pre-conditions | *Argument* -- every parameter is non-`null`. +
  *Argument* -- every parameter is non-empty.
| Errors
| <<type_SymmetricCrypto,`SymmetricCrypto`>> -- if an internal error occurred. +
  <<type_SymmetricCryptoIO,`SymmetricCryptoIO`>> -- if an IO error occurred when processing a command.
|===
